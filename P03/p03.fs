...
let inline lowerCharToInt c = int c - int 'a' + 1
let inline upperCharToInt c = int c - int 'A' + 27

let mapPriority (c: Char) =
    match c with
    | c when c >= 'a' && c <= 'z' -> lowerCharToInt c
    | c when c >= 'A' && c <= 'Z' -> upperCharToInt c
    | _ -> 0

let toCharArray (s: string) = s.ToCharArray()

let calcPriority (s: Char[]) =
    let midPoint = s.Length / 2
    let c1, c2 = Set.ofArray s[.. midPoint - 1], Set.ofArray s[midPoint..]
    let common = Set.intersect c1 c2

    common |> Set.toSeq |> Seq.head |> mapPriority

let sumPriorities () =
    input.Split("\n") |> Seq.map toCharArray |> Seq.map calcPriority |> Seq.sum

let findBadge items =
    items
    |> Seq.skip 1
    |> Seq.fold (fun b i -> Set.intersect b i) (Seq.head items)
    |> Set.toSeq
    |> Seq.head

let sumBadgePriorities () =
    input2.Split("\n")
    |> Seq.map toCharArray
    |> Seq.map Set.ofArray
    |> Seq.chunkBySize 3
